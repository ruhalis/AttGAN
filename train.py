...
def main():
    ...
    # Force CUDA if GPU is available and args.gpu is True
    ...
    if args.data == 'CelebA':
        ...
        valid_dataset = CelebA(args.data_path, args.attr_path, args.img_size, 'train', args.attrs)
    if args.data == 'CelebA-HQ':
        ...
        valid_dataset = CelebA_HQ(args.data_path, args.attr_path, args.image_list_path, args.img_size, 'train', args.attrs)
    
    # Check if datasets are not empty
    if len(train_dataset) == 0:
        raise RuntimeError(f"No training images found in {args.data_path}")
    ...
    valid_dataloader = data.DataLoader(
        valid_dataset, batch_size=min(args.n_samples, len(valid_dataset)),
    ...
    # Get the first batch more safely
    try:
        valid_iter = iter(valid_dataloader)
        fixed_img_a, fixed_att_a = next(valid_iter)
    except StopIteration:
        print("Warning: Could not get validation batch. Using a training batch instead.")
        train_iter = iter(train_dataloader)
        fixed_img_a, fixed_att_a = next(train_iter)
